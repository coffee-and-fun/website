...
Only five files carry frontmatter at all: `404.liquid` and the four utility pages (`CNAME.njk`, `robots.njk`, the Apple Pay verification blob, and one non-blog markdown page). Everything else relies on Eleventy's default `foo.liquid` to `/foo/index.html` mapping.
...
### Checking the SEO score

Every indexable page is meant to score **100** on Lighthouse's SEO category, and there is a script to prove it:
...
It builds the URL list from `docs/` itself, so a new page is covered as soon as it is built, and it exits non-zero if anything scores below 100. Pass URLs to narrow it: `npm run seo -- /apps/ /`.

Lighthouse 12's SEO category is ten scored audits (`is-crawlable`, `document-title`, `meta-description`, `http-status-code`, `link-text`, `crawlable-anchors`, `robots-txt`, `image-alt`, `hreflang`, `canonical`) plus `structured-data`, which is manual and does not affect the number. All ten are decided by markup this repo controls, so a regression is always fixable.

**Two pages are meant to fail `is-crawlable`** and are allowlisted in the script: `/stop-wasting/` is the "website blocked" screen ProcrastiNOT shows, and `/404.html`. Neither belongs in a search index, and both are correctly absent from `sitemap.xml`. The allowlist forgives *only* `is-crawlable` on those two, so any other audit failing there still fails the run.

`sitemap.xml` is generated by `tools/build-sitemap.mjs` and should always equal the number of indexable pages. If they diverge, something is either missing from search or listed when it should not be.

---

## The sitemap

`docs/sitemap.xml` is **generated after the build from the pages Eleventy actually wrote**, by `tools/build-sitemap.mjs`, hooked into `eleventy.after`. There is no sitemap template any more.

It used to be `src/pages/sitemap.njk`, looping over `collections.all`. That looped over *source files*, one step removed from what ships, so it could not see two things:

- **Whether a page is indexable.** `stop-wasting` carries `<meta name="robots" content="noindex, nofollow">` and was listed anyway, telling Google to index and not index the same page.
- **What the page claims as canonical.** The template built its own URLs from `site.url + item.url`, so a canonical that disagreed went unnoticed.

Reading the built HTML fixes both by construction: **the URL in the sitemap IS the page's canonical**, and a noindex page can never appear.

### lastmod is driven by content, not by dates

Google only trusts `lastmod` if it is consistently accurate and ignores the signal entirely when it is not. The two obvious sources are both wrong here:

- **Frontmatter `date`** is the publication date. Edit a 2015 post today and the sitemap still says 2015. This is what the old template shipped.
- **Git commit date** inflates. One site-wide sweep, an em dash pass or a schema injection, touches every file and would claim all 103 pages changed at once.

So each page's rendered HTML is hashed instead, with the volatile parts stripped (asset `?v=` query strings, comments, whitespace). **The date only moves when that hash moves.** The ledger lives in `tools/sitemap-lastmod.json` and is committed, so dates survive across builds and machines.

The baseline was seeded once: blog posts kept their publication dates, other pages took the date of the restructure they had just received.

### Things that will bite you

- **Production builds only.** A dev build skips the `htmlmin` transform, so its HTML hashes differently for reasons unrelated to content. `eleventy.after` returns early unless `ELEVENTY_ENV=production`, and the last production sitemap simply stays in place while you work.
- **A running `eleventy --serve` will race the build.** Watchers rewrite `docs/` in dev mode whenever a source file changes. If the generator finds unminified pages during a production build it **refuses to update the ledger** and says so, rather than stamping today's date onto pages that never changed. If you see that warning, stop the watchers and rebuild.
- `changefreq` and `priority` are gone. Google ignores both, and they were 208 lines of noise. The file went from 23,136 to 12,518 bytes.
...
Rebuilds the sitemap from whatever is in `docs/` and reports, without touching the ledger. Exits non-zero on any problem, so it belongs in CI. It fails on a duplicate canonical, an indexable page with no canonical, or a canonical pointing off-site. Eleventy swallows writes made from inside an `eleventy.after` hook, so this is how the result is actually visible.
